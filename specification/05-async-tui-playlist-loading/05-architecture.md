...
| `TUIPlaylist::track_from_path` | `tui/src/ui/model/playlist.rs:157-178` | Shallow wrapper: 3 lines of logic around `Track::read_track_from_path` with a code annotation acknowledging it should not exist. No value added beyond error context. | Pass: if deleted and replaced by gRPC-supplied metadata, callers simplify (no disk I/O, no error handling for file reads). |
